...
func runCompile(args []string) int {
	...
	// M4-cloud: when the target opts into hybrid-dev mode, emit HCL
	// (versions/backend/main/iam) + an env.template + a README with the
	// 3-step run instructions. Skipped for non-hybrid targets — they
	// stay compose-only as in Phase 1.
	if rp.Plan.Targets[*target].CredsPassthrough {
