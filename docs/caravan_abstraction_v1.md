# caravan Abstraction Recommendation
...
2. A cloud-only list and how caravan signals it.
...
|---|---|---|
| `function` | **Keep** | Lambda↔FastAPI is *Moderate* (File 4). caravan ships a `handler` decorator + Mangum-style wrapper for both directions. |
| `bucket` | **Keep** | S3↔MinIO is *Trivial* — endpoint URL env var. The cleanest primitive in the set. |
| `topic+sub` | **Keep, with split** | SNS↔localstack is *Trivial*; SQS↔ElasticMQ is *Trivial*. Treat `topic` and `queue` as separate primitives — they're distinct AWS services and caravan users routinely want a queue without a topic. Suggest **`topic` + `queue`** as two primitives. |
| `cron` | **Keep** | EventBridge Scheduler is *Moderate*. caravan generates a `@caravan.cron("0 2 * * *")` decorator that emits the AWS schedule rule and a local APScheduler container only if explicitly enabled. |
| `secret` | **Keep** | SSM Parameter Store + Secrets Manager are both *Trivial*. caravan ships `secret("name")` that resolves at runtime against either AWS or env vars locally. |

**Recommended addition**: **`kv`** (DynamoDB). Difficulty *Trivial*; nearly every Python web app needs a session store, idempotency key store, or feature-flag store; DDB-local is one of the best-supported emulators in the ecosystem. Adding it costs caravan ~2 days of implementation and unlocks a huge class of apps. The original 5-primitive scope omitted this and would force users to roll their own.

**Recommended addition**: **`db.sql`** (Postgres). Difficulty *Trivial*; ~70% of Python apps need a relational DB. Aurora/RDS↔postgres container is just a DSN swap. Even if caravan doesn't manage the connection (let SQLAlchemy do that), it needs to know the resource exists so it can provision the cluster + inject `DATABASE_URL`.

**Revised PoC primitive list (7)**:
- `function` (Lambda or Fargate task — caravan picks based on workload hint)
- `bucket` (S3 / MinIO)
- `queue` (SQS / ElasticMQ)
...
From File 4's Intractable bucket (~15 services), caravan must support declaring a resource and refusing to bind it locally. Proposal:

```python
# Example: caravan IR (Python decorator style)
@caravan.resource(
    type="bedrock.llm",
...
Behavior:
- `caravan up --target=aws` → provisions Bedrock invocation permissions, injects the boto3 Bedrock client wrapper.
- `caravan up --target=local` → **errors loudly**: "Resource LLM is `cloud_only=True`; pass `--allow-cloud-only=LLM` to use the real AWS service from local dev, or implement a `LocalLLM` adapter."
...
For each, caravan ships a `cloud_only=True` resource type. No local backend. Documented loudly.
...
```yaml
# caravan.yaml — top-level config in repo root
name: my-app
default_target: local           # used when `caravan up` is run with no --target
...
- **The yaml itself** = the IR. caravan parses this into a protobuf spec.
...
- **GHA layer** = generated workflow files (`.github/workflows/deploy-staging.yml`) that run `caravan up --target=aws-staging` on the listed trigger. Just enough to bootstrap CI; users can edit by hand if they need more.
...
| S3 ↔ minio | Lifecycle policies use different DSLs; caravan only emits to one side. | caravan generates S3 lifecycle XML for AWS; emits the equivalent minio command for local — but flagged as best-effort. |
...
| Step Functions ↔ aws-stepfunctions-local | Distributed Map state, Express semantics, and intrinsic-function library have version drift with the local container. | Pin to a known good local container version; flag in caravan output when ASL uses a feature the pinned local doesn't support. |
...
- [ ] `caravan up --target=local` brings up the full local stack via docker-compose. App runs against it.
- [ ] `caravan up --target=aws-staging` provisions equivalent AWS resources via Pulumi. Same app code runs.
...
>
> ~22 AWS↔local pairs are **Trivial** (env-driven `endpoint_url` or DSN swap) — these are caravan's bread and butter. ~12 are **Moderate** (need a thin adapter). ~5 are **Hard** (Cognito, Step Functions multi-service, API Gateway WebSocket, fan-out durability) — caravan defines Protocol interfaces and users pick the impl. ~15 are **Intractable** (CloudFront, Bedrock, SageMaker, IoT Defender, etc.) — caravan marks them `cloud_only=True` and refuses local binding.
>
> The yaml switch lives at `caravan.yaml`; a `--target=` CLI flag flips between `local` (docker-compose generated), `aws-staging` and `aws-prod` (Pulumi in-process). GHA workflows are generated for CI. No code changes per environment — only env vars injected by the caravan runtime.
>
> The 15-pair "Intractable" list is the *honest scope boundary* of caravan. Trying to abstract any of them would produce false positives — code that works locally and fails in production. Treat the list as a feature of the abstraction, not a limitation.
