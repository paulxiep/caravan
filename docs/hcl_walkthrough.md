...
**Modules** — reusable HCL bundles (`module "x" { source = "..."; ... }`). Caravan's v1 emit is flat files (no `module {}` blocks) for legibility.
...
**Phase 2 source scan** cross-checks: `./embedder/src/lib.rs` must contain `provide(Embedder, ...)` (matching yaml's `seams.Embedder.path`); `./api/src/lib.rs` contains `client::<Embedder>(...)`. Phase 4 uses target's seam decisions to compute `CARAVAN_RPC_PEERS` per deploy unit.
...
Files caravan emits into `infra/prod-split/generated/`. Driven by the yaml above.

### 3a. `main.tf`

```hcl
# === Generated by caravan compile --target=prod-split. DO NOT EDIT. ===
...
# Bedrock is a managed service that caravan just grants IAM access to.
...
# (out-of-band aws ssm put-parameter); caravan only emits the IAM grant + env-var injection.
...
# (Tag values are git-sha-based; caravan populates at build time. Compile-only step
#  doesn't pin a tag — that's filled by `caravan build` + `caravan up`.)

# ----- CloudWatch log groups (one per container) -----
# Lambda log groups follow /aws/lambda/<func> convention but caravan emits them
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      CARAVAN_RPC_SELF               = "api"
      CARAVAN_RPC_PEERS              = jsonencode({
        Embedder = {
          mode         = "lambda"
          function_url = aws_lambda_function_url.embedder.function_url
        }
      })
      CARAVAN_RPC_SHARED_SECRET      = random_password.rpc_secret.result
    }
  }
}
...
resource "aws_lambda_function" "embedder" {
  ...
  environment {
    variables = {
      ...
      AWS_REGION                = var.region
      CARAVAN_RPC_SELF           = "embedder"
      CARAVAN_RPC_PEERS          = jsonencode({})       # no outbound peers (provider only)
      CARAVAN_RPC_SHARED_SECRET  = random_password.rpc_secret.result
    }
  }
}
...
- **`CARAVAN_RPC_PEERS` is keyed by interface name** (`Embedder`), not peer container name. Single provider per interface (PoC constraint).
- **No `CARAVAN_RPC_BUNDLE` env var** — collapsed into `CARAVAN_RPC_SELF` (the container name).
...
- Each container's role gets policy statements for everything in its `uses:` list. Resource-type → action mapping is caravan-owned (e.g. `search` → `aoss:APIAccessAll`; `llm` → `bedrock:InvokeModel`).
...
Files caravan emits into `infra/dev-monolith/generated/`. Driven by the `dev-monolith` target in the yaml above. Single container `monolith` packs both `./api` and `./embedder` source dirs.

```yaml
# === Generated by caravan compile --target=dev-monolith. DO NOT EDIT. ===
...
services:
  ...
  ollama:
    ...
    # Models pulled on first request; caravan can optionally pre-pull via a one-shot sidecar.

  # ----- container `monolith` -----
  # Both api and embedder source dirs in one image. CARAVAN_RPC_PEERS routes Embedder
  ...
  monolith:
    ...
    environment:
      ...
      CARAVAN_RPC_SELF: "monolith"
      CARAVAN_RPC_PEERS: |
        { "Embedder": { "mode": "inproc" } }
      CARAVAN_RPC_SHARED_SECRET: "dev-only"
    depends_on:
      opensearch:
...
**`dev-split` (not shown) would emit two compose services** (`api` and `embedder`) instead of one `monolith`, with the api's `CARAVAN_RPC_PEERS` set to `{"Embedder": {"mode": "http", "url": "http://embedder:8080"}}` — standard compose service-name hostname.
