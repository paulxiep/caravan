...
These dimensions are orthogonal. A yaml `target:` names a point in (packaging × placement × composition). A repo can declare many such targets — `dev`, `hybrid-dev`, `staging`, `prod`, `pr-preview` — and `caravan up --target=<name>` flips between them. Same source code everywhere.
