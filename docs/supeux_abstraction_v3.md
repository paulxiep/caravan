# supeux Abstraction Recommendation — v3 (three-language re-derivation)
...
2. **The "supeux ships no runtime adapter library" call (v2 §4) is reinforced.** Three languages now have mature Tier 1 community libraries — `litellm` (Python), `rig` (Rust), **Vercel AI SDK** (TS) — without any supeux-authored code in user space. Same for token verification (`authlib` / `jsonwebtoken` / `jose`) and email (`smtplib` / `lettre` / `nodemailer`). The pattern generalizes: when three independent ecosystems converge on different-named libraries that solve the same Tier 1 pair, supeux's job is curation, not implementation.
...
- **Per-language adapter libraries.** Community libraries (rig / litellm / Vercel AI SDK / etc.) cover most Tier 1 hard pairs today. `supeux-adapters-*` may ship for proven gaps; not currently a priority.
...
**The current authorship call: rely on mature community libraries where they exist; supeux ships `supeux-adapters-*` only for proven gaps.** Three independent ecosystems converging on different-named libraries that solve the same Tier 1 pair is strong evidence that the per-language landscape is mature enough for supeux to curate rather than implement:
...
**Tier 2 — No-local-stand-in services (~17 Rust / ~15 Python / ~15 TypeScript pairs)**: "cloud-only" is short for *no OSS engine reproduces this service locally*. The `cloud_only:` IR flag is a **provisioning marker** — it tells supeux not to generate a local docker-compose stand-in. What happens depends on which of four patterns the user picks per service:
...
**On supeux-authored libraries**: today the per-pair landscape points toward community-library sufficiency for Tier 1 *across three independent languages*; supeux ships zero code libraries and curates guidance instead. The three-language convergence is the strongest evidence yet that this call is right.
...
**Switching**: `supeux up --target=local` (or `--target=staging`, etc.). The CLI flag flips environments; the yaml decides what each environment maps to. **No code change** between environments — only env vars injected by the supeux runtime into containers / Lambda environment / etc. TS users read these env vars identically: `process.env.S3_ENDPOINT_URL`, `process.env.DATABASE_URL`, etc.
...
2. **Pulumi-TS exists** (unlike Pulumi-Rust), so TS users *could* go Pulumi-TS. supeux's HCL emission still wins for cross-language reviewability and security-team auditability — the same audit story that wins in Python and Rust.
3. **CDKtf was sunset and archived Dec 10, 2025** by HashiCorp/IBM, closing the language-native HCL-emitting alternative. Reinforces v3 §5's HCL-emission call: TS users now have no first-party HCL-from-TS toolchain; supeux fills that gap polyglot-first.
4. **Lambda Node 22 runtime is the mature default**; Bun support is community/experimental (`bun-lambda` custom runtime); Deno container-baseline-only. supeux defaults to Node 22 for v1 reference apps; Bun/Deno work as Fargate containers (user-opt-in) since they're "any container with a Dockerfile."
...
8. **SST is the native-TS IaC competitor**. SST is TS-only with TS-engine + TS-component layer; supeux is polyglot user code with Go-engine + yaml-component layer. Different audience; no zero-sum competition (same posture as Shuttle.rs for Rust).
9. **TS source → JS bundle is user-side**. Lambda Node runtime expects `.js`; users bundle with esbuild / tsc / `bun build` before packaging. supeux does NOT bundle — same posture as "supeux doesn't build your container."
...
| **v1.1** | `topic`, `kv` primitives; `cron` triggers; **Go reference app**; `supeux logs` + `supeux exec`; **Tier 1 gap survey** across all four languages | Closes the "all 8 stateful primitives" gap and the four-language first-class set. Decides whether `supeux-adapters-*` ships any code. |
...
- **Lambda cold starts vary per language.** Rust container-image Lambdas typically start in <100 ms; TS (Node 22) in 100–500 ms; Python in 500–1500 ms; Java in seconds. supeux defaults are runtime-agnostic; users tune memory / provisioned concurrency / SnapStart via `overrides:` per target.
- **TS runtime variance.** Node 22 is the official Lambda runtime; **Bun on Lambda is community/experimental** (`bun-lambda` custom runtime); Deno is container-baseline-only. supeux defaults to Node 22 for the v1 PoC; Bun and Deno work fine on Fargate as "any container with a Dockerfile" but ship without per-runtime supeux guidance in v1.
...
| TS source → JS bundle (TS-specific) | Lambda Node runtime expects `.js`; users must bundle (esbuild / tsc / `bun build`) before packaging. | Document the bundle step; supeux does NOT bundle for the user (same posture as "supeux doesn't build your container"). |

---

## 13. One-page summary

> **End-state vision (§2)**: supeux is a containers-first deploy tool. One yaml manifest, any first-class language (Python, Rust, TypeScript today; Go closing the set), any cloud (AWS first; GCP/Azure reachable later). User code is unmodified containers wired to env-var-driven endpoints; supeux generates docker-compose locally, Terraform/OpenTofu HCL for cloud, and GHA workflows for CI. No SDK, no runtime coupling, no language lock-in. 8 primitives (`service`, `bucket`, `queue`, `topic`, `kv`, `db.sql`, `secret`, `static_site`), 3 `service` shapes (`long-running`, `function`, `batch`), 5 trigger types, ~20 cloud-only resource types, hand-written-`.tf` extension model.
>
> **What v3 adds vs v2** (§1, §4, §7b, §12): TypeScript evidence — three independent ecosystems agreeing on the same architectural answer is the strongest signal yet that the design is right. TS Tier 0/1/2 headcounts (~22 / ~5 / ~15) sit between Python and Rust; the design holds. Three Tier 1 community libraries (Vercel AI SDK, `jose`, `nodemailer`) confirm the "supeux ships no runtime adapter library" call across three languages. CDKtf's Dec 10, 2025 sunset (HashiCorp/IBM) closes the last language-native HCL-emit alternative for TS, reinforcing supeux's HCL-emit posture. Per-language gotchas (Prisma `binaryTargets`, `kafkajs` SigV4 gap, Bun-on-Lambda experimental, TS-source-bundling) added to §12.
>
...
>
> **The IR** (§6): one `supeux.yaml` projecting to three artifacts: `docker-compose.generated.yaml` (local), `infra/<target>/*.tf` (cloud), `.github/workflows/deploy-<target>.yml` (CI). Switching is a single `--target=` flag. No code change between environments — only env vars injected by the supeux runtime.
>
...
**Current (pre-edit)** lists Python + Rust evidence files, `aws_service_groups.md`, `cloud_providers.md`, `supeux_abstraction_v1.md`, `supeux_abstraction_v2.md`.

**Applied**: added `mapping_aws_to_typescript.md`, `mapping_typescript_to_aws.md`, `typescript_api_diffs.md`. Added `supeux_abstraction_v3.md` as "long-form derivation that produced this thesis (three-language re-derivation)". Demoted v2 to historical record line.
...
### 14.5 [`supeux_abstraction_v2.md:75`](supeux_abstraction_v2.md) end-state language coverage
...
**Disposition (2026-05-16): ACCEPTED.** Thesis.md:73 and README.md:5 are edited to make the emit/apply split explicit. Final verb: **`supeux compile`** (matches the "application-definition compiler" framing in README and thesis, and avoids the awkward `spec` (singular IR dump) vs `specs` (verb) homophone). `supeux up` errors out by default if no spec has been emitted for the target; `supeux up --regenerate` is the opt-in one-shot flow.

**Original (pre-edit) — [`thesis.md:73`](thesis.md)** (Positioning section):
> "Write one yaml describing the module graph and its bound cloud resources; run `supeux up --target=<name>` and it deploys — to a cloud target (via auditable Terraform/HCL under the hood) or to a local docker-compose stack."

**Original (pre-edit) — [`README.md:5`](../README.md)** (top-of-file framing):
> "Write one yaml describing the module graph and its bound cloud resources; run `supeux up --target=<name>` and it deploys to a cloud target (via auditable Terraform/HCL under the hood) or to a local docker-compose stack."

**Why this needed fixing**: both original wordings imply `supeux up` is a one-shot command that emits and applies in one step, with the HCL "under the hood" — i.e., a transient byproduct. v3 §5 establishes that the actual model is **two-step on purpose**: `supeux compile` emits HCL into `infra/<target>/generated/` as a reviewable on-disk artifact, then `supeux up` applies the already-emitted spec. "Auditable HCL" is operationally true precisely *because* there's a review window between emit and apply, not despite it. The original wording undersold this — it made the HCL sound incidental when it's actually the load-bearing artifact.
...
**Verb-naming history** (preserved for future readers): v3 drafts used `supeux specs` (plural) to distinguish from the existing `supeux spec` (singular IR dump per `architecture-brainstorm.md` §7). Alternatives considered: `supeux build`, `supeux emit`, `supeux generate`. `supeux compile` won because (a) it mirrors README's "application-definition compiler" framing, (b) the noun-vs-verb distinction (`spec` reads, `compile` writes) is sharper than the plural-vs-singular distinction, and (c) it carries the right mental model: source yaml → compiled HCL/compose artifact → execution.

---

*Conceptual home: `thesis.md`. Historical record: `supeux_abstraction_v1.md` (Python-decorator-SDK framing), `supeux_abstraction_v2.md` (containers-first re-derivation with Python + Rust). v3 supersedes v2 by re-deriving with three first-class languages and confirming the design holds.*
